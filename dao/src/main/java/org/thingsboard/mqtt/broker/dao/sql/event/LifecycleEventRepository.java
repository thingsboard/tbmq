...
import org.thingsboard.mqtt.broker.dao.model.sql.event.LifecycleEventEntity;
...
public interface LifecycleEventRepository extends EventRepository<LifecycleEventEntity, LifecycleEvent>, JpaRepository<LifecycleEventEntity, UUID> {
    ...
    @Override
    @Query(nativeQuery = true, value = "SELECT * FROM lc_event e WHERE e.entity_id = :entityId ORDER BY e.ts DESC LIMIT :limit")
    List<LifecycleEventEntity> findLatestEvents(@Param("entityId") UUID entityId, @Param("limit") int limit);
    ...
    Page<LifecycleEventEntity> findEvents(@Param("entityId") UUID entityId,
