...
import org.thingsboard.mqtt.broker.dao.model.sql.event.StatisticsEventEntity;
...
public interface StatisticsEventRepository extends EventRepository<StatisticsEventEntity, StatisticsEvent>, JpaRepository<StatisticsEventEntity, UUID> {
    ...
    @Override
    @Query(nativeQuery = true, value = "SELECT * FROM stats_event e WHERE e.entity_id = :entityId ORDER BY e.ts DESC LIMIT :limit")
    List<StatisticsEventEntity> findLatestEvents(@Param("entityId") UUID entityId, @Param("limit") int limit);
    ...
    Page<StatisticsEventEntity> findEvents(@Param("entityId") UUID entityId,
    ...
    void removeEvents(@Param("entityId") UUID entityId,
                      @Param("startTime") Long startTime,
                      @Param("endTime") Long endTime);
