...
import org.thingsboard.mqtt.broker.dao.model.sql.event.ErrorEventEntity;
...
public interface ErrorEventRepository extends EventRepository<ErrorEventEntity, ErrorEvent>, JpaRepository<ErrorEventEntity, UUID> {
    ...
    @Override
    @Query(nativeQuery = true, value = "SELECT * FROM error_event e WHERE e.entity_id = :entityId ORDER BY e.ts DESC LIMIT :limit")
    List<ErrorEventEntity> findLatestEvents(@Param("entityId") UUID entityId, @Param("limit") int limit);
    ...
    Page<ErrorEventEntity> findEvents(@Param("entityId") UUID entityId,
