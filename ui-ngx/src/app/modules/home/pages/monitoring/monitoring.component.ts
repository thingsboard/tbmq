...
export class MonitoringComponent extends PageComponent {
  ...
  private pushShiftLatestValue(data: TimeseriesData, chartType: string) {
    ...
    if (this.charts[chartType].data.datasets[0].data &&
        latestValue.ts > this.charts[chartType].data.datasets[0].data[this.charts[chartType].data.datasets[0].data.length-1]?.ts) {
