...
@Slf4j
@DirtiesContext
@ContextConfiguration(classes = SingleLevelTopicPubSubIntegrationTest.class, loader = SpringBootContextLoader.class)
public class SingleLevelTopicPubSubIntegrationTest extends AbstractPubSubIntegrationTest {
    ...
    @Test
    public void testPubSubSingleLevelTopic() throws Throwable {
        initPubSubTest();
    }
    ...
    void initPublisher(Waiter waiter, int publisherId) {
        try {
            MqttClient pubClient = new MqttClient("tcp://" + mqttAddress + ":" + mqttPort, "single_level_topic_pub_client_" + publisherId);
            pubClient.connect();
            String topic = TOPICS.get(publisherId % TOPICS.size());
    ...
    void initSubscriber(Waiter waiter, int subscriberId) {
        try {
            MqttClient subClient = new MqttClient("tcp://" + mqttAddress + ":" + mqttPort, "single_level_topic_sub_client_" + subscriberId);
            subClient.connect();
            ...
            subClient.subscribe(SINGLE_LEVELS_TOPIC_FILTER, qos, (topic, message) -> {
