...
public class TotalThroughputQuotaDevicePersistedIntegrationTestCase extends AbstractTotalThroughputQuotaIntegrationTest {
    ...
    public void givenDeviceBacklogPaidForAtPublish_whenQuotaIsDry_thenWholeBacklogIsReplayedUnmetered() throws Throwable {
        ...
        pub.disconnect();
        pub.close();
        // a fixed wait, deliberately: the backlog must reach the persisted store before the drain, and "stored" has
        // no counter to poll - the subscriber is offline, so nothing observable moves. Draining too early would
        // charge the tail against an empty bucket and fail the assertion below for the wrong reason.
        Thread.sleep(1000);
