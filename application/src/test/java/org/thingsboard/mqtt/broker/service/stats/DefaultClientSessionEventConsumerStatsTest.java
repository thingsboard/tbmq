...
public class DefaultClientSessionEventConsumerStatsTest {

    private DefaultClientSessionEventConsumerStats stats;

    @Before
    public void setUp() {
        StatsFactory statsFactory = new DefaultStatsFactory(new SimpleMeterRegistry());
        stats = new DefaultClientSessionEventConsumerStats("consumer-1", statsFactory);
    }

    @Test
    public void givenNoPacksProcessed_whenGetAvgPackSize_thenReturnsZeroNotNaN() {
        // Session events are sparse (and split across 2 consumers by default), so idle intervals
        // with a 0 divisor are common. The avg must be a clean 0, not NaN, in the log line.
