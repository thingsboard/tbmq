...
public class MqttClientCredentialsBulkImportService {
    ...
    public BulkImportResult<MqttClientCredentials> processBulkImport(BulkImportRequest request) throws Exception {
        ...
        for (int i = 0; i < records.size(); i++) {
            try {
                ...
                // Map columns based on the request mapping
                for (int j = 0; j < request.getMapping().getColumns().size(); j++) {
                    ...
                    setCredentialsField(credentials, columnMapping.getType(), value);
                }
    ...
    private void setCredentialsField(MqttClientCredentials entity, BulkImportColumnType type, String value) {
        ...
        switch (type) {
            ...
            case CREDENTIALS_TYPE:
                entity.setCredentialsType(ClientCredentialsType.valueOf(value.toUpperCase()));
                break;
            case CREDENTIALS_VALUE:
                entity.setCredentialsValue(value);
                break;
            case DESCRIPTION:
    ...
    private MqttClientCredentials saveOrUpdate(MqttClientCredentials credentials, boolean update) {
        // Logic to check for existing credentials by Name (TBMQ usually enforces unique names)
        ...
        if (existing.isPresent()) {
            ...
            return existing.get(); // Skip update if not requested
        } else {
