...
public class MqttWssHandlerProvider {

    @Value("${listener.wss.config.protocol}")
    private String wssProtocol;
    ...
    private SSLContext createSslContext() {
        try {
            ...
            if (StringUtils.isEmpty(wssProtocol)) {
                wssProtocol = "TLS";
            } else {
                if (log.isDebugEnabled()) {
                    log.debug("sslProtocol is set to {}", wssProtocol);
                }
            }

            SSLContext sslContext = SSLContext.getInstance(wssProtocol);
